...
public class ChatAdapter extends RecyclerView.Adapter<ChatAdapter.ViewHolder> { //RecyclerAdapter


        private List<Conversations> mListItems; // List
        private Context mContext;
        private OnItemClickListener mListener; // Listener for the OnItemClickListener interface

        //constructor
    public ChatAdapter(List<Conversations> listItems, Context context) { // constructor
        ...
        public ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {// this method calls when ever our view method is created , ie; the instance of ViewHolder class is created
            View view = LayoutInflater.from(parent.getContext())
                    .inflate(R.layout.list_chat, parent, false); /**list_item-> is the Card view which holds the data in the recycler view*/
...
//                view.setLayoutParams(layoutParams);
        ...
        public void onBindViewHolder(@NonNull ViewHolder holder, int position) {//populate the data into the list_item (View Holder), as we scroll
            ...
            Conversations listItem = mListItems.get(position);
            if ((listItem.getFrom().equals(uID))){
                holder.messageText.setBackground(ContextCompat.getDrawable(mContext, R.drawable.outgoing_bubble));
                holder.messageText.setTextColor(Color.WHITE);
                holder.messageText.setTextAlignment(View.TEXT_ALIGNMENT_TEXT_END);
            }else {
                holder.messageText.setBackground(ContextCompat.getDrawable(mContext, R.drawable.incoming_bubble));
                holder.messageText.setTextColor(Color.BLACK);
                holder.messageText.setTextAlignment(View.TEXT_ALIGNMENT_TEXT_START);
            }
            holder.messageText.setText(listItem.getMessage());
